...
class AppStore extends React.Component {
  ...
  getFilteredAppsList = () => {
    ...
    const filteredAppsList = appsList.filter(eachApp => 
        eachApp.category === activeTabId && eachApp.appName.toLowerCase().includes(searchInput.toLowerCase())
      )
    ...
    return (
      <div className="app-store-container">
        ...
        <div className='search-input-container'>
            <input onChange={this.updateSearchInput} className='input' type='search' placeholder='Search'/>
            <img alt='search icon' src="https://assets.ccbp.in/frontend/react-js/app-store/app-store-search-img.png" className='search-icon'/>
        </div>
        <ul className='tabs-container'>
            {tabsList.map((eachTab)=> <TabItem key={eachTab.tabId} tabDeatails={eachTab} updateTabId={this.updateTabId} isActiveTab={eachTab.tabId === activeTabId} />)}
        </ul>
        <ul className='apps-container'>
            {
                filteredAppsList.map(eachApp => <AppItem key={eachApp.appId} appDetails={eachApp}/>)
            }
        </ul>
      </div>
    )
